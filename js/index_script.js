...
let graph1 = document.querySelector('.p-100');
let graph2 = document.querySelector('.p-50');
let graphText1 = document.querySelector('#item1 p');
let graphText2 = document.querySelector('#item2 p');
let personaImg = document.querySelectorAll('#img_container img');
let symbolImg = document.querySelector('#sector7 img');
...
observer2.observe(typewriter[8]);
observer2.observe(typewriter[9]);
observer2.observe(graph1);
observer2.observe(graph2);
observer2.observe(graphText1);
observer2.observe(graphText2);
...
observer2.observe(personaImg[0]);
observer2.observe(personaImg[1]);
observer2.observe(personaImg[2]);
observer2.observe(symbolImg);
...
nav_btn.forEach((item) => {
    item.addEventListener('click', (event) => {
        if(event.target.classList.contains('bg')) {
            ...
            window.scrollTo({top: 15500, behavior: 'smooth'});
        }
        else if(event.target.classList.contains('fn')) {
            window.scrollTo({top: 19440, behavior: 'smooth'});
        }
    })
})
...
window.addEventListener('scroll', () => {
    ...
    let newSize = scrollTop * 0.05 - 500;
    console.log(scrollTop); //Dev Only
